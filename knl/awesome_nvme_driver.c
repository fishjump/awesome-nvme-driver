...
MODULE_DESCRIPTION("Minimal NVMe stub driver for BAR access");

#define DRIVER_NAME "nvme_stub"
#define DEVICE_NAME "my_nvme_stub"

static struct pci_device_id nvme_stub_ids[] = {
    { PCI_DEVICE(0x15ad, 0x07f0) }, // VMware NVMe Controller
    { 0 }
};
MODULE_DEVICE_TABLE(pci, nvme_stub_ids);

static int nvme_stub_probe(struct pci_dev *pdev, const struct pci_device_id *id);
static void nvme_stub_remove(struct pci_dev *pdev);
static struct pci_driver nvme_stub_driver = {
    .name = DRIVER_NAME,
    .id_table = nvme_stub_ids,
    .probe = nvme_stub_probe,
    .remove = nvme_stub_remove,
};

module_pci_driver(nvme_stub_driver);


static const struct file_operations nvme_stub_fops = {
    .owner = THIS_MODULE,
};

struct nvme_stub_dev {
    struct pci_dev *pdev;
    void __iomem *bar0;
...
static struct nvme_stub_dev *stub_dev;
...
static int nvme_stub_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    // 1. 分配设备结构体
    stub_dev = devm_kzalloc(&pdev->dev, sizeof(*stub_dev), GFP_KERNEL);
    if (!stub_dev)
        return -ENOMEM;
    stub_dev->pdev = pdev;
